...
import { Paper, TextField, Typography, Pagination, FormControlLabel, Checkbox, Box, ToggleButtonGroup, ToggleButton, Button, Drawer, IconButton } from '@mui/material';
...
const RecipeSearchPage = () => {
    ...
    const [caloriePreference, setCaloriePreference] = useState<'more' | 'less'>('less');
    const [proteinPreference, setProteinPreference] = useState<'more' | 'less'>('less');
    ...
    const getRecipes = () => {
        ...
        if (useCalorieFilter && calories !== null) {
            filters.push(`calories${caloriePreference === 'more' ? '.from=' : '.to='}${calories}`);
        }
        if (useProteinFilter && protein !== null) {
            filters.push(`protein_percentage${proteinPreference === 'more' ? '.from=' : '.to='}${protein}`);
        }
        if (useFatFilter && fat !== null) {
            filters.push(`fat_percentage${fatPreference === 'more' ? '.from=' : '.to='}${fat}`);
        }
        if (useCarbFilter && carbs !== null) {
            filters.push(`carbs_percentage${carbPreference === 'more' ? '.from=' : '.to='}${carbs}`);
        }

        const filterString = filters.length ? `&${filters.join('&')}` : '';

        axios.post(`http://localhost:5000/recipes/search/v3?format=json&search_expression=${value}&page_number=${pageNumber - 1}&max_results=${resultsPerPage}${filterString}`)
            .then((response) => {
    ...
    }, [pageNumber, value, calories, protein, fat, carbs, 
        caloriePreference, proteinPreference, fatPreference, carbPreference, 
        useCalorieFilter, useProteinFilter, useFatFilter, useCarbFilter]);
    ...
    return (
        <div style={{ padding: '24px' }}>
            ...
            {/* Diet Presets */}
            <Box style={{ marginBottom: '16px', display: 'flex', justifyContent: 'center' }}>
                <Button variant="contained" onClick={() => applyDietPreset('Keto')}>Keto</Button>
                <Button variant="contained" onClick={() => applyDietPreset('Paleo')} style={{ marginLeft: '8px' }}>Paleo</Button>
                <Button variant="contained" onClick={() => applyDietPreset('Mediterranean')} style={{ marginLeft: '8px' }}>Mediterranean</Button>
            </Box>

            {/* Filter Button */}
            <Box style={{ marginBottom: '16px', display: 'flex', justifyContent: 'center' }}>
                <IconButton onClick={() => setDrawerOpen(true)} style={{ marginBottom: '16px' }} >
                    <MenuIcon />
                </IconButton>
            </Box>
                            ...
                            <Checkbox
                                ...
                                onChange={(event) => {
                                    ...
                                    if (!event.target.checked) {
                                        setCalories(null); // Reset calories when filter is turned off
                                    }
                                }}
                            />
                        }
                        ...
                        control={
                            <Checkbox
                                ...
                                onChange={(event) => {
                                    ...
                                    if (!event.target.checked) {
                                        setProtein(null); // Reset protein when filter is turned off
                                    }
                                }}
                            />
                        }
                        ...
                        control={
                            <Checkbox
                                ...
                                onChange={(event) => {
                                    ...
                                    if (!event.target.checked) {
                                        setFat(null); // Reset fat when filter is turned off
                                    }
                                }}
                            />
                        }
                        ...
                        control={
                            <Checkbox
                                ...
                                onChange={(event) => {
                                    ...
                                    if (!event.target.checked) {
                                        setCarbs(null); // Reset carbs when filter is turned off
                                    }
                                }}
                            />
                        }
